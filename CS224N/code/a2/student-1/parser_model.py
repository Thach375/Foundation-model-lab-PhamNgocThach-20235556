...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        # take embedding for each word index in w
        x = self.embeddings[w] # shape: (batch_size, n_features, embed_size)
        
        # reshape x to (batch_size, n_features * embed_size)
    ...
    def forward(self, w):
        ...
        # first layer
        hidden = torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias  # shape: (batch_size, hidden_size)
        hidden = F.relu(hidden)
        hidden = self.dropout(hidden)
        # output layer
        logits = torch.matmul(hidden, self.hidden_to_logits_weight) + self.hidden_to_logits_bias  # shape: (batch_size, n_classes)
